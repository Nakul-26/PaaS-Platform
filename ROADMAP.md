...
Phase 2 Task 1 (NATS infrastructure + subject/message contract) is done — see the Phase 2 row above. Next: Task 2, the `0007_nodes_containers.sql` migration + `NodeRepository`/`ContainerRepository` (can run in parallel with Task 3, worker registration/heartbeat over NATS, since neither depends on the other).
