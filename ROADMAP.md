...
Phase 2 Tasks 1-2 (NATS infrastructure + subject/message contract; `nodes`/`containers` migration + repositories) are done — see the Phase 2 row above. Next: Task 3, worker node registration + heartbeat over NATS (can run in parallel with Task 4, the assignment subscription, once Task 3 exists — both depend only on Task 1).
