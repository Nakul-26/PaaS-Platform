...
| Data access, per aggregate | `OrganizationRepository`, `ApplicationRepository`, `DeploymentRepository`, `NodeRepository`, etc. | `internal/db` (one file per aggregate) | `pgx`/`sqlc`-generated Postgres queries | A different store for a specific aggregate if one ever outgrows Postgres (e.g. `resource_usage_samples` → Timescale, flagged in `ARCHITECTURE.md` §9) | `apiserver`, `controller-manager` |
